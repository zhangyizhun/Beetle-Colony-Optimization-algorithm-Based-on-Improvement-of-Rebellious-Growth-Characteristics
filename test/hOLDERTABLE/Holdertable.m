...
%holdertable  最小值为-19.2085  在8.05502,9.66459 正负互相搭配
  ...
  for j=1:sizepop
    %天牛子群部分位置移动
       d0=step/c;%两须之间的距离
        ...
        %天牛子群部分最佳位置移动
       d0=step/c;%两须之间的距离
        xleftgb=gbestgbso+V(j,:)*d0/2;
        fleftgb=funbas(xleft(1,1),xleftgb(1,2));
        xrightgb=gbestgbso-V(j,:)*d0/2;
        frightgb=funbas(xright(1,1),xrightgb(1,2));
        Ygb(j,:)=step.*V(j,:).*sign(fleftgb-frightgb);
